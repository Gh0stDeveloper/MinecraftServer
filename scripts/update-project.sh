...
mkdir -p "$APP_DIR" "$ROOT/scripts" "$ROOT/addons"; rsync -a --delete --exclude='.git' --exclude='*.zip' --exclude='*.tar.gz' "$SRC/" "$APP_DIR/"; rsync -a --delete "$APP_DIR/scripts/" "$ROOT/scripts/"; rsync -a --delete "$APP_DIR/addons/" "$ROOT/addons/"
...
systemctl restart bedrock-web.service 2>/dev/null || true; ok "Proyecto, configuración de motores y web actualizados desde main."
