...
PUBLIC_IP="${PUBLIC_IP_OVERRIDE:-$TEMPLATE_IP}"; [[ -n "$DOMAIN" ]] || DOMAIN="$TEMPLATE_DOMAIN"
domain_matches_ip(){ local domain="$1" expected="$2"; [[ -n "$domain" && -n "$expected" ]] || return 1; getent ahostsv4 "$domain" 2>/dev/null | awk '{print $1}' | grep -Fxq "$expected"; }
if [[ -z "$HOST" ]]; then
  if domain_matches_ip "$DOMAIN" "$PUBLIC_IP"; then HOST="$DOMAIN"; echo "[mcserver] DNS verificado: $DOMAIN -> $PUBLIC_IP"
  elif [[ -n "$PUBLIC_IP" ]]; then HOST="$PUBLIC_IP"; [[ -n "$DOMAIN" ]] && echo "[mcserver] DNS aún no apunta a $PUBLIC_IP; se usará la IP como fallback."
  elif [[ -r /dev/tty ]]; then read -r -p 'IP o dominio público de Minecraft: ' HOST </dev/tty
  else echo 'Usa --host o configura PUBLIC_IP/PUBLIC_DOMAIN.' >&2; exit 1; fi
fi
