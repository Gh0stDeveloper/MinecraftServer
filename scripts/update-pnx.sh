...
gradle_version(){
  local src="$1"
  cd "$src"
  ./gradlew --no-daemon --console=plain --no-configuration-cache --version
}
...
build_pnx_from_source(){
  ...
  ui_step "No se encontró disponible el snapshot oficial; usando el commit fijado ${PNX_SOURCE_REF:0:12}"
  git -C "$src" init -q
  git -C "$src" remote add origin "$PNX_SOURCE_REPO"
  if ! git -C "$src" fetch -q --depth 1 origin "$PNX_SOURCE_REF"; then rm -rf "$src"; die "No se pudo descargar el commit oficial fijado de PowerNukkitX."; fi
  ...
  chmod +x "$src/gradlew"
  if ! ui_run_task "Descargando/preparando Gradle" gradle_version "$src"; then rm -rf "$src"; return 1; fi
  if ! ui_run_task "Compilando release de PowerNukkitX $PNX_EXPECTED_VERSION" gradle_shadow "$src"; then rm -rf "$src"; return 1; fi
...
meta="$(curl --http1.1 -fsSL --retry 3 --connect-timeout 15 "$meta_url" 2>/dev/null || true)"
...
ui_step "Buscando snapshot oficial de PowerNukkitX"
if ! curl --http1.1 -fsSL --connect-timeout 20 --max-time 300 "$PNX_DOWNLOAD_URL" -o "$tmp" 2>/dev/null; then
  warn "Snapshot oficial no disponible (404 o fallo upstream); compilando desde el commit fijado."
  rm -f "$tmp"
  tmp="$(mktemp "$CACHE_DIR/pnx.XXXXXX.jar")"
  build_pnx_from_source "$tmp"
  source_kind="source-${PNX_SOURCE_REF:0:12}"
else
  ok "Snapshot oficial descargado"
fi
...
ok "PowerNukkitX actualizado a $release"
ui_note "Backup: $backup"
