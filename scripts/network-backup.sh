...
backup_network(){
  ...
  out="$BACKUP_DIR/network-$stamp"
  mkdir -p "$out"
  log "Backup consistente: $out"
  for instance in "${INSTANCES[@]}"; do
    target="$INSTANCES_DIR/$instance"
    [[ -d "$target" ]] || continue
    local -a keep=()
    for item in server.properties allowlist.json permissions.json worlds behavior_packs resource_packs development_behavior_packs development_resource_packs; do
      [[ -e "$target/$item" ]] && keep+=("$item")
    done
