...
if instances_by_engine pnx | grep -q .; then
  ...
  [[ -f "$PNX_CURRENT_LINK/powernukkitx-shaded.jar" ]] && ok "PowerNukkitX $(current_pnx_version) instalado" || { warn 'PowerNukkitX no instalado'; fail=$((fail+1)); }
fi
...
if [[ -x "$APP_DIR/scripts/minigame-manager.sh" ]]; then "$APP_DIR/scripts/minigame-manager.sh" status || true; fi
