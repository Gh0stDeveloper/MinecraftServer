...
function joinQueue(player, mode) {
  ...
  purgeQueue(mode);
  QUEUES[mode].push(player.id);
  player.sendMessage(`§aEntraste a la cola de ${GAME} ${LABEL[mode]}. §f${QUEUES[mode].length}/${REQUIRED[mode]}`);
  tryStart(mode);
}
...
function tryStart(mode) {
  ...
  for (const player of players) {
    player.sendMessage(`§6Partida encontrada: §f${GAME} ${LABEL[mode]}`);
    player.sendMessage("§7ArenaManager pendiente de configurar con el mapa físico.");
  }
}
...
async function menu(player) {
  const response = await new ActionFormData()
    .title(`§l${GAME}`)
    .body("Selecciona modalidad")
    .button(`§aSolo\n§7${REQUIRED.solo} jugadores`)
    .button(`§eDuo\n§7${REQUIRED.duo} jugadores`)
    .button(`§cEscuadra\n§7${REQUIRED.squad} jugadores`)
    .button("§7Salir de la cola")
    .show(player);
...
world.beforeEvents.chatSend.subscribe((event) => {
  ...
  if (msg === "!play" || msg === "!menu") { event.cancel = true; const player = event.sender; system.run(() => menu(player).catch(() => {})); }
});
