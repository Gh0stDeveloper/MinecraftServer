...
def valid_admin_token(token: str | None) -> bool:
    if not token or len(token) > 256 or not token_configured():
        return False
...
def validate_uploaded_archive(path: Path) -> str | None:
    """Return a human-readable error when an uploaded .zip/.mcworld is not a real ZIP."""
    try:
        with path.open("rb") as fh:
            head = fh.read(8)
    except OSError as exc:
        return f"No se pudo validar el archivo subido: {exc}"

    if zipfile.is_zipfile(path):
        try:
            with zipfile.ZipFile(path, "r") as archive:
                if not archive.infolist():
                    return "El archivo ZIP/.mcworld está vacío."
        except (OSError, zipfile.BadZipFile) as exc:
            return f"El archivo ZIP/.mcworld está dañado: {exc}"
        return None

    if head.startswith(b"7z\xbc\xaf'\x1c"):
        return "El archivo es 7z aunque su nombre termina en .zip/.mcworld. Créalo otra vez usando formato ZIP estándar."
    if head.startswith(b"Rar!\x1a\x07"):
        return "El archivo es RAR aunque su nombre termina en .zip/.mcworld. Créalo otra vez usando formato ZIP estándar."
    if head.startswith((b"PK\x03\x04", b"PK\x05\x06", b"PK\x07\x08")):
        return "El archivo parece ZIP, pero está incompleto o corrupto: no contiene un directorio central ZIP válido. Vuelve a comprimir el mundo y súbelo de nuevo."
    return "El contenido subido no es un ZIP/.mcworld estándar válido. No basta con cambiar la extensión; vuelve a comprimir la carpeta del mundo como ZIP."
...
class Handler(BaseHTTPRequestHandler):
    ...
    def admin_ok(self) -> bool:
        if not self.secure_admin_request():
            self.json_response({"ok": False, "error": "El panel administrativo requiere HTTPS."}, 403)
            return False
        if valid_admin_token(self.headers.get("X-Admin-Token")):
            return True
        self.json_response({"ok": False, "error": "Token administrativo inválido o no configurado."}, 401)
        return False
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/admin/check":
            if not self.admin_ok():
                return
            self.json_response({"ok": True, "max_upload_mb": MAX_UPLOAD_MB})
            return
        if parsed.path == "/api/admin/survival/import-status":
            ...
            if len(ident) != 32 or any(c not in "0123456789abcdef" for c in ident):
                self.json_response({"ok": False, "error": "ID inválido."}, 400)
                return
            path = RESULT_DIR / f"{ident}.json"
            if not path.is_file():
                self.json_response({"ok": True, "id": ident, "state": "queued"})
                return
            try:
                payload = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, json.JSONDecodeError):
                payload = {"ok": False, "id": ident, "state": "unknown", "error": "Estado no disponible."}
    ...
    def do_POST(self):
        parsed = urlparse(self.path)
        if parsed.path != "/api/admin/survival/upload":
            self.json_response({"ok": False, "error": "Endpoint no encontrado."}, 404)
            return
        if not self.admin_ok():
            return
        ...
        try:
            ...
            length = 0
        if length <= 0:
            self.json_response({"ok": False, "error": "Archivo vacío o Content-Length inválido."}, 411)
            return
        if length > MAX_UPLOAD_BYTES:
            self.json_response({"ok": False, "error": f"El archivo supera el límite de {MAX_UPLOAD_MB} MB."}, 413)
            return

        original = Path(self.headers.get("X-Filename", "survival.zip")).name
        suffix = Path(original).suffix.lower()
        if suffix not in {".zip", ".mcworld"}:
            self.json_response({"ok": False, "error": "Solo se admiten archivos .zip o .mcworld."}, 415)
            return
        ...
        final_path = UPLOAD_DIR / f"{ident}{suffix}"
        part_path = UPLOAD_DIR / f".{ident}{suffix}.part"
        ...
        try:
            with part_path.open("wb") as fh:
                while remaining:
                    ...
                    if not chunk:
                        raise ConnectionError("La conexión terminó antes de completar la subida.")
                    fh.write(chunk)
                    digest.update(chunk)
                    remaining -= len(chunk)
                fh.flush()
            ...
            archive_error = validate_uploaded_archive(part_path)
            if archive_error:
                part_path.unlink(missing_ok=True)
                self.json_response({"ok": False, "error": archive_error}, 422)
                return
            ...
            self.json_response({"ok": False, "error": f"No se pudo guardar la subida: {exc}"}, 500)
            return
