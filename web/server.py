...
import json, mimetypes, os, socket, struct, time
from http.server import BaseHTTPRequestHandler, ThreadingHTTPServer
from pathlib import Path
from urllib.parse import unquote, urlparse
BASE = Path(os.environ.get("BEDROCK_BASE", "/opt/bedrock-network"))
STATIC = Path(__file__).resolve().parent / "static"
HOST = os.environ.get("WEB_HOST", "0.0.0.0"); PORT = int(os.environ.get("WEB_PORT", "8080"))
...
def ping_bedrock(port:int, timeout:float=.35)->dict:
    sent=int(time.time()*1000); packet=b"\x01"+struct.pack(">Q",sent)+MAGIC+struct.pack(">Q",0x4E45584F52414244)
    sock=socket.socket(socket.AF_INET,socket.SOCK_DGRAM); sock.settimeout(timeout)
    try: sock.sendto(packet,("127.0.0.1",port)); data,_=sock.recvfrom(4096)
    except OSError: return {"online":False,"players":0,"max_players":None,"version":None,"motd":None}
    finally: sock.close()
    if len(data)<35 or data[0]!=0x1C: return {"online":True,"players":None,"max_players":None,"version":None,"motd":None}
    try:
        n=struct.unpack(">H",data[33:35])[0]; parts=data[35:35+n].decode("utf-8",errors="replace").split(";")
    ...
    except (ValueError,struct.error): return {"online":True,"players":None,"max_players":None,"version":None,"motd":None}
def read_state(name:str):
    path=BASE/"state"/name
    return path.read_text(encoding="utf-8").strip() if path.exists() else None
def status_payload()->dict:
    env=parse_env(BASE/"config"/"network.env"); engines=parse_env(BASE/"config"/"engines.env")
    ...
    for instance in INSTANCES:
        ...
        port=int(props.get("server-port",env.get(f"{instance.upper()}_PORT","19132"))); ping=ping_bedrock(port)
...
class Handler(BaseHTTPRequestHandler):
    server_version="BedrockNetworkWeb/1.1"
    def log_message(self,fmt,*args): print(f"[web] {self.address_string()} {fmt % args}")
    def common_headers(self):
        self.send_header("X-Content-Type-Options","nosniff"); self.send_header("X-Frame-Options","DENY"); self.send_header("Referrer-Policy","no-referrer"); self.send_header("Permissions-Policy","camera=(), microphone=(), geolocation=()"); self.send_header("Cache-Control","no-store")
